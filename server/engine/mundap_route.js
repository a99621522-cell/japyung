...
async function 문답처리(입력, 설정 = {}) {
  try {
    ...
    const 공통 = { 년도: new Date().getFullYear(), 출생연도: 입력.출생연도, 성별: 입력.성별, 관법: 입력.관법 === '궁통보감' ? '궁통보감' : undefined };
    ...
    let 본문 = await 부르기(브리프);
    if (!본문 || 본문.length < 5) return { 성공: false, 사유: '모델이 빈 답을 돌려주었습니다' };
    ...
    try {
      ...
      검사 = D.검사(본문, { 브리프, 모드: 검사모드, 궁통있음: /\[궁통보감 조건절 —/.test(브리프) });
      if (!검사.통과) {
        const 본문2 = await 부르기(브리프 + '\n' + 검사.다시쓰기지시);
        if (본문2 && 본문2.length >= 5) {
          const 검사2 = D.검사(본문2, { 브리프, 모드: 검사모드 });
          if (검사2.오류.length < 검사.오류.length) { 본문 = 본문2; 검사 = 검사2; 검사.다시씀 = true; }
        }
      }
    } catch (e) { /* 검사기가 없어도 답은 나간다 */ }

    return { 성공: true, 본문, 모드, 설명요청, 출처: 'gemini',
